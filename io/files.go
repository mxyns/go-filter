...
func FormatImageTempName(temp_path, input_file_path *string, filter_name string) (new_name, new_path string) {
	...
	// input_file_path = ./some_folder/some.input.file.png
	shards := strings.Split(*input_file_path, "/") // [. some_folder input.png]
	filename := shards[len(shards)-1]              // some.input.file.png

	// temp_path = ./some_other_folder/output_folder/46513213676574.png
		...
		case *dRequests.FileRequest:
			{
				...
				_, new_path := FormatImageTempName(&temp_file_path, input_file_path, strings.ReplaceAll(filtersCmds[i], " ", "-"))
